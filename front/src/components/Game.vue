...
import type { MovieTemplate, Choice, Character, Ending } from '../types/movie';
import CharacterAvatar from './ui/CharacterAvatar.vue';
import { Home as HomeIcon, ArrowLeft } from 'lucide-vue-next';
...
const updateSpotlight = (e: PointerEvent, active: boolean) => {
  ...
  el.style.setProperty('--mg-spotlight-color', active ? 'rgba(168, 85, 247, 0.25)' : 'rgba(255, 255, 255, 0.1)');
};
 ...
 */
const historyStack = useStorage<{nodeId: string, state: any}[]>('mg_history_stack', []);
...
const checkEnding = (nodeId: string) => {
    ...
    if (data.endings && data.endings[nodeId]) {
        emit('end', {
            ...data.endings[nodeId],
            endingKey: nodeId,
            nodeId,
    ...
    if (endingKey) {
        ...
        emit('end', {
            type: 'neutral',
            description: node?.content?.text?.trim() || '故事结束',
            endingKey,
            nodeId,
            reachedAt: new Date().toISOString(),
        });
...
onMounted(() => {
    if (!props.data?.nodes) return;
    if (!currentNodeId.value || (!props.data.nodes[currentNodeId.value] && !(props.data.endings && props.data.endings[currentNodeId.value]))) {
        resetToStart();
    }
...
watch(
    ...
    (next) => {
        ...
        if (!currentNodeId.value || (!next.nodes[currentNodeId.value] && !(next.endings && next.endings[currentNodeId.value]))) {
            resetToStart();
        }
    },
    { immediate: true },
...
const missingNode = computed(() => {
    ...
    if (props.data.endings && props.data.endings[currentNodeId.value]) return false;
...
const currentAgents = computed(() => {
    ...
    if (currentNode.value?.characters && currentNode.value.characters.length > 0) {
        currentNode.value.characters.forEach((name, idx) => {
            const n = (name || '').trim();
            if (!n) return;
            const char = Object.values(props.data?.characters || {}).find(c => c.name === n || c.id === n);
            if (char) {
                agents.push(char);
                return;
            }
    ...
    if (agents.length === 0) {
        const seed = (props.data?.projectId || props.data?.title || 'mg').split('').reduce((a, b) => a + b.charCodeAt(0), 0);
...
const normalizeGenderTag = (raw?: string) => {
    ...
    if (/(female|woman|girl|\bf\b|女|女性|女生|女士|女人)/.test(g)) return 'Female';
...
const handleChoice = (choice: Choice) => {
    ...
    const canonicalizeEndingId = (raw: string) => {
        ...
        if (v === 'good_end' || v === 'end_good' || v === 'good') return 'ending_good';
        if (v === 'neutral_end' || v === 'end_neutral' || v === 'neutral') return 'ending_neutral';
        return v;
    };
    ...
    if (props.data?.endings && props.data.endings[nextId]) {
         emit('end', {
...
const getEmotion = (agent: Character) => {
    ...
    const text = (currentNode.value?.content?.text || '').toLowerCase();
    ...
    const hash = (currentNodeId.value + agent.name).split('').reduce((a, b) => a + b.charCodeAt(0), 0);
    return emotions[hash % emotions.length];
};

const backgroundImageBase64 = computed(() => (props.data?.backgroundImageBase64 || '').trim());
  ...
  <div class="h-screen w-full bg-black text-white overflow-hidden relative font-sans">
      ...
      <main class="relative z-10 h-full flex flex-col items-center justify-end md:justify-center pb-0 md:pb-20">
          ...
          <div class="flex-1 w-full flex items-center justify-center relative">
              ...
              >
                  <div v-for="agent in currentAgents" :key="agent.id" class="flex flex-col items-center mx-4 md:mx-12 group relative">
                       <div class="relative">
                       ...
                       <div class="absolute top-0 left-1/2 -translate-x-1/2 translate-y-[calc(-100%-12px)] opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200">
                          <div class="min-w-[220px] max-w-[260px] bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl px-4 py-3 shadow-2xl">
                            <div class="text-sm font-semibold text-white/95">{{ agent.name }}</div>
                            <div class="mt-1 text-xs text-white/70">
                              <span v-if="agent.role">{{ agent.role }}</span>
                              <span v-if="agent.role && agent.age"> · </span>
                              <span v-if="agent.age">{{ agent.age }}岁</span>
                            </div>
                          </div>
                       </div>
                  </div>
          ...
          <div class="w-full max-w-4xl px-4 pb-8 md:pb-0 z-20">
              <div
                ref="spotlightCardEl"
                class="mg-spotlight bg-black/70 backdrop-blur-xl border border-white/10 rounded-2xl md:rounded-3xl p-6 md:p-10 shadow-2xl ring-1 ring-white/5 relative overflow-hidden"
              ...
              >
                  ...
                  <div class="min-h-[100px] mb-8 flex items-center justify-center">
                      ...
                      >
                          <p :key="currentNodeId" class="text-lg md:text-2xl leading-relaxed font-light text-neutral-100 text-center drop-shadow-md">
                              {{ currentNode?.content?.text || (data ? '...' : '没有可用的游戏数据，请返回首页重新生成或导入。') }}
                          </p>
                      </Transition>
                  </div>
                  ...
                  <div class="flex flex-col gap-3">
                       <button v-for="(choice, idx) in availableChoices" 
                               ...
                               class="group relative w-full text-left bg-white/5 hover:bg-white/10 border border-white/10 hover:border-purple-500/50 p-4 md:p-5 rounded-xl transition-all duration-300 transform hover:translate-x-1 active:scale-[0.99] overflow-hidden"
                               :style="{ animationDelay: `${idx * 100}ms` }"
                       >
                           <div class="absolute inset-0 bg-gradient-to-r from-purple-600/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                           <div class="flex items-center gap-4 relative z-10">
                               <span class="text-neutral-300 group-hover:text-white text-base md:text-lg font-light tracking-wide transition-colors">
                                  {{ choice.text }}
                               </span>
                           </div>
                       </button>
