...
- 层级递进：后续节点的 `level` 必须是当前节点的 level +1 的节点, 或者结局节点
- 层级宽度：每层最多 **5 个节点**, 最少 **1 个节点**
- 层级数量: 最多 15 层, 最少 12 层
- 层级分布：
    - 每个 level 至少 2 个节点。
    - 允许收束：必须允许 **至少 10%** 的 level 只有 1 个节点 (剧情收束点)。
- 结局一致性：所有结局 (`endings`) 视为处于最后一个 Level。

## 3. 节点复用与收束 (关键)
- 多对一结构：并不是每个节点的选项都必须指向全新的节点, 即允许节点被多个节点引用
...
- 游离节点: **绝对禁止** 出现任何从 start 节点开始到达不了的节点
- 选项数量: 数量为 1~3 个
- 选项分布：
    - 1 个选项的节点：**< 10%**
    - 2 个选项的节点：**> 60%**
