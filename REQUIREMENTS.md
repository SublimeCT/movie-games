...
3.  **剧情分析弹窗**: 展示剧情元信息、分享状态与关键标识；当剧情处于已分享状态时提供“取消分享”入口。
4.  **剧情树**:
...
*   **URL**: `POST /import`
*   **功能**: 接收前端导入的 `MovieTemplate`，进行节点/结局/图结构等数据清理后保存到数据库（写入 `glm_requests.processed_response`，并标记 `template_source=import`），返回可编辑的 `requestId`。
*   **参数**:
...
*   **状态**: 已移除。
*   **说明**: 原有的好感度系统（`affinityEffect`, `mg_affinity_state`）已从前端与后端逻辑中彻底移除。角色表情现在基于节点内容与哈希回退机制推断。
...
**效果**:
*   **引擎**: 使用 `Vue Flow` (`@vue-flow/core`) 替代原有的 SVG 手写实现，提供更流畅的交互体验和更强的扩展性。
*   **布局**: 保持原有的分层布局算法，将计算出的坐标直接传递给 Vue Flow 节点。
*   **交互**:
    *   支持平移拖拽、滚轮缩放、双指缩放（移动端）。
    *   **自适应视图**: 初始加载及重置时自动调用 `fitView` 适配容器大小。
    *   **移动端适配**: 移动端自动优化初始缩放比例和位置，确保内容居中或靠左易读。
*   **节点渲染**:
    *   使用自定义节点组件 (`CustomNode.vue`) 复现原有的节点样式（玻璃拟态、高亮光晕、类型颜色区分）。
    *   节点内容包括：类型标签（NODE/ENDING）、节点 ID/名称、高亮状态指示。
*   **连线**:
    *   支持路径高亮：选中节点或结局时，高亮从根节点到该节点的完整路径（紫色/粉色发光效果）。
